...
class Simulate {
 public:
  ...
  std::deque<mjtNum*> rEE_d_traj;

  mjtNum lEE[3];
  mjtNum rEE[3];
  std::deque<mjtNum*> lEE_traj;
  std::deque<mjtNum*> rEE_traj;
  ...
  void visualizeTraj_mppi(const std::deque<std::deque<mjtNum*>>& traj, mjvScene& scn, mjtNum size[], float color[])
  {
    for(int j=0; j<traj.size(); j++)
    {
      for(int i = 1; i < traj[j].size(); i++) 
      {
          mjvGeom* test_geom = scn.geoms + scn.ngeom++;
          mjv_initGeom(test_geom, mjGEOM_SPHERE, size, NULL, NULL, color);
          mjv_connector(test_geom, mjGEOM_LINE, 2, traj[j][i-1], traj[j][i]);
      }
    }
  }
