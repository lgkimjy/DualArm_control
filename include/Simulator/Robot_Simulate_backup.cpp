...
void MakeCommandSection(mj::Simulate* sim) {
  // create command slider + on/off button
  mjui_add(&sim->ui1, sim->defCmd);
}
////////////////////////////// JY CODE ////////////////////////////// 
/////////////////////////////////////////////////////////////////////
...
void AlignAndScaleView(mj::Simulate* sim, const mjModel* m) {
  ...
  ////////////////////////////// JY CODE //////////////////////////////
    // front view 1
  sim->cam.azimuth = 145;
  sim->cam.elevation = -15;
  // front view 2
  // sim->cam.azimuth = -180;
  // sim->cam.elevation = -16;
  // behind view
  // sim->cam.azimuth = 44;
  // sim->cam.elevation = -16;
  // // side view
  // // sim->cam.azimuth = 88.2;
  // // sim->cam.elevation = -20.7;

  sim->cam.distance = 1.25 * m->stat.extent;
...
#define X(name)                                                  \
  ...
  // visualizeGeom(next_foot, new mjtNum[3]{0.02, 0.02, 0.1}, new float[4]{1.0, 0.0, 0.5, 0.5}, mjGEOM_CYLINDER);  // Visual for Next Foot
  // visualizeGeom(com, new mjtNum[3]{0.05, 0.05, 0.5}, new float[4]{1.0, 0.0, 0.0, 0.5}, mjGEOM_SPHERE);          // Visual for CoM
  // visualizeGeom(lEE_d, new mjtNum[3]{0.02, 0.02, 1.0}, new float[4]{1.0, 1.0, 0.0, 1.0}, mjGEOM_SPHERE);          // Visual for CoM
  // visualizeGeom(rEE_d, new mjtNum[3]{0.02, 0.02, 1.0}, new float[4]{1.0, 1.0, 0.0, 1.0}, mjGEOM_SPHERE);          // Visual for CoM

  visualizeTraj(lEE_d_traj, scn, new mjtNum[3]{0.5, 0.05, 0.5}, new float[4]{1.0, 0.0, 1.0, 1.0});
  visualizeTraj(rEE_d_traj, scn, new mjtNum[3]{0.5, 0.05, 0.5}, new float[4]{1.0, 1.0, 1.0, 1.0});
  visualizeTraj(lEE_traj, scn, new mjtNum[3]{0.5, 0.05, 0.5}, new float[4]{0.3, 0.1, 1.0, 0.5});
  visualizeTraj(rEE_traj, scn, new mjtNum[3]{0.5, 0.05, 0.5}, new float[4]{0.5, 1.0, 0.3, 0.5});
  ////////////////////////////// JY CODE ////////////////////////////// 
  /////////////////////////////////////////////////////////////////////
